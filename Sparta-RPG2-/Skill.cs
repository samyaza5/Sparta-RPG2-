...
namespace Sparta_RPG2_
{
    public class Skill
    {
            ...
            while (true)
            {
                ...
                switch (input)
                {
                    ...
                    case "1":
                        ...
                        if (player.SP > 0)
                        {
                            Console.WriteLine($"{passiveSkill.SkillList[0].Name}을 습득했습니다.");
                            Console.WriteLine($"최대HP {player.MaxHP} -> {player.MaxHP * 1.2f} ");
                            passiveSkill.SkillList[0].GetSkill = true;
                            player.MaxHP = (int)Math.Round(player.MaxHP * 1.2f);
                            player.HP = player.MaxHP;
                            player.SP--;
                            passiveSkill.SkillList[0].SkillLv += 1;
                            mySkill.Add(passiveSkill.SkillList[0]);
                        }
                        else
                        {
        ...
        private static void SkillManager(Character player, PassiveSkill passiveSkill)
        {
            ...
            foreach(PassiveSkill skill in mySkill)
            {
                int idx = 1;
                Console.WriteLine($"{idx}. {skill.Name} - {skill.Description} ({skill.SkillLv}/{skill.MaxSkillLv})");idx++;
            }
